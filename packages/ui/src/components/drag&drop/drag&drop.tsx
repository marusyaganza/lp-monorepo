...
const Drop = ({children, dropEffect}: PropsWithChildren<DropProps>) => {
    ...
    return (
        //@ts-ignore
        <div className="answerBox" onDrop={dropHandler} onDragOver={dragOverHandler}>{children}</div>
    )
}
...
const Drag = ({data, children, dropEffect}: PropsWithChildren<DragProps>) => {
    ...
    return (
        <span className="draggable" draggable onDragStart={dragStartHandler}>{children}</span>
    )
};
