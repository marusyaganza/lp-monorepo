...
exports.getImgUrl = exports.getAudioUrl = exports.getDefs = exports.formatDictionaryEntity = exports.formatHw = exports.formatReplace = exports.isNumber = exports.formatArray = exports.filterString = void 0;
...
function formatComplexTag(str) {
    ...
    const resultArr = rawArr.map(el => {
        ...
        const matchingTag = constants_1.COMPLEX_TAGS.find(tag => element.includes(tag.opening));
        if (matchingTag) {
 ...
 */
function removeComplexTags(sample) {
    const tag = constants_1.COMPLEX_TAGS_TO_REMOVE.find(tag => sample.includes(tag.opening));
    if (!tag) {
        return sample;
    }
...
function formatReplace(str, tags) {
    let result = str;
    tags.forEach(tag => {
...
function formatHw(hw) {
    return formatReplace(hw, constants_1.HW_TAGS);
}
exports.formatHw = formatHw;
/**
 ...
 * @returns html string
 */
function formatDictionaryEntity(str) {
    if (!str) {
        return;
    }
    const result = formatReplace(str, constants_1.TAGS);
    return formatComplexTag(result);
}
exports.formatDictionaryEntity = formatDictionaryEntity;
// TODO retrieve translation for examples for Spanish language
// {
//   t: 'el idioma ingl\u00e9s',
//   tr: 'the English language'
// }
function getDefs(def, cognateRef) {
    const result = [];
...
function getAudioUrl(audio, lang) {
    if (!audio) {
        return;
    }
